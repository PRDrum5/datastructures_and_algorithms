...
void test_append_linked_list(void) {
    ...
    int status = append(list, new_value);
    TEST_ASSERT_EQUAL(2, list->length);
    TEST_ASSERT_EQUAL(v, list->head->value);
    TEST_ASSERT_EQUAL(new_value, list->head->next->value);
    TEST_ASSERT_EQUAL(0, status);

    const int another_value = 5;
    status = append(list, another_value);
    TEST_ASSERT_EQUAL(3, list->length);
    TEST_ASSERT_EQUAL(v, list->head->value);
    TEST_ASSERT_EQUAL(new_value, list->head->next->value);
    TEST_ASSERT_EQUAL(another_value, list->head->next->next->value);
    TEST_ASSERT_EQUAL(0, status);
...
int main(void) {
    ...
    RUN_TEST(test_append_linked_list);
